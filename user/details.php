...
<body style="background-color: #f1f3f4">
    ...
    <!-- Container Start-->
    <div class="container">
        <h1 class="mb-5 text-center">Drawer Details STR-01</h1>
        <a href="view-history.php" class="float-end mb-3">View Borrowing History</a>
        <div>
            <table class="text-center table-bordered table table-light">
                ...
                <tbody>
                    <?php
                    $sql = mysqli_query($conn, "SELECT * FROM dataoeminjaman ORDER BY id DESC");
                    if (mysqli_num_rows($sql) == 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($sql)) { // fetch query yang sesuai ke dalam array
                            if ($row['value'] == 0) {
                                ...
                                $borrower = "Sigit Indriyanto/582344";
                                echo '
                      <tr>
                        <td scope="row">' . $no . '</td>
                        <td>' . $row['tools'] . '</td>
